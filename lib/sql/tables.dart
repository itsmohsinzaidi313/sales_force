class TablesV1{
  ...
  static String SALESMAN = 'CREATE TABLE salesman(id integer primary key, category_to_salesman_id text, product_category_id text, user_id text)';

  static String APP_SETTINGS = 'CREATE TABLE app_settings(id integer primary key, sync_date text)';

  static String PRODUCTPRICES = 'CREATE TABLE product_prices(id integer primary key, product_to_customer_group_id text, product_id text, customer_group_id text, cash_price text, credit_price text)';

  static String CUSTOMERGROUPS = 'CREATE TABLE customer_groups(id integer primary key, customer_group_id text, name text)';

  static String CUSTOMER = 'CREATE TABLE customer(id integer primary key,customer_id text, customer_group_id text, user_id text, country_id text, city_id text, state_id text, area_id text, customer_first_name text, customer_last_name text, customer_email text, customer_phone text, customer_mobile text, customer_shop_name, customer_address1 text, status text, discount_type text, discount text, credit_limit text)';

  static String ORDER_MASTER = 'CREATE TABLE order_master(id integer primary key, user_id text, customer_id text, order_amount text, order_discount text, order_total text, order_status text, order_delivery_date text, createdon text, is_upload integer)';

  static String ORDER_DETAIL = 'CREATE TABLE order_detail(id integer primary key, master_id integer, product_category_id text, product_id text, order_product_total_packs text, order_product_price_per_pack text, order_product_discount_per_pack text, order_product_discounted_pack_price text, order_product_total_discount text, order_product_total_price text, product_image text)';

  static String VISITS = 'CREATE TABLE visits(id integer primary key, customer_id text, user_id text, lat text, long text, isorder text, createdon text, is_upload integer, pair_id integer)';

  static String CATEGORY_PERMISSIONS = 'CREATE TABLE category_permissions(id integer primary key, category_id text, user_id text)';
